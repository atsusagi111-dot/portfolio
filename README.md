...
- [ ] **公式 LINE を追加**（友だち追加ボタン・リンク。URL を教えてください）
- [ ] **サービス「ホームページ制作」に「実績を見る」を追加**し、実績ページの「Minato Piano Atelier｜HP制作」へ飛べるようにする（`data/services.ts` の `relatedWorkId` を使う）
- [ ] **「Minato Piano Atelier｜HP制作」の制作画面をもう少し大きく**
...
| `/privacy/` | プライバシーポリシー |
| （全ページ右下） | チャットウィジェット（FAQボタン＋自由入力。外部AIは使わず `data/faq.json` から回答。仕組みは `docs/chatbot.md`） |
...
自動コミットのフックは 2026-09-21 に無効化しました（`.claude/hooks/stop-auto-commit-push.sh` は残してありますが、`.claude/settings.json` から外しています）。
コミットと push は、オーナーが指示したときだけ行います。`main` へ push すると Vercel の本番（旧サイト）が置き換わる点は変わりません。
