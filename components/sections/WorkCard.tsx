...
export function WorkCard({
  ...
  const steps = [
    { label: worksSection.challengeLabel, en: "Challenge", text: work.challenge },
  ...
  return (
    <article
      id={work.id}
      className="overflow-hidden rounded-2xl border border-navy-50 bg-surface"
    >
      <div className="p-6 sm:p-10">
        ...
        <ol className="mt-8 grid gap-4 lg:grid-cols-3">
          {steps.map((step, index) => (
            <li key={step.label} className="relative rounded-xl bg-surface-alt p-6">
              <p className="flex items-baseline gap-3">
                <span className="font-en text-lg text-gold-600">{step.en}</span>
              ...
              {index < steps.length - 1 && (
                <span
                  aria-hidden="true"
                  className="absolute -bottom-5 left-1/2 z-10 -translate-x-1/2 rotate-90 text-xs text-gold lg:-right-3.5 lg:bottom-auto lg:left-auto lg:top-1/2 lg:-translate-y-1/2 lg:translate-x-0 lg:rotate-0"
                >
                  ▶
                </span>
              )}
            ...
            <div className="mt-4 grid gap-6 sm:grid-cols-2">
              {work.screenshots.map((shot) => (
                <figure key={shot.src}>
                  ...
                  <div className="relative aspect-square overflow-hidden rounded-xl border border-navy-50 bg-surface-alt">
                    <Image
                      src={shot.src}
                  ...
                  <figcaption className="mt-2 text-xs text-ink-muted">{shot.label}</figcaption>
                </figure>
              ))}
            </div>
          </div>
